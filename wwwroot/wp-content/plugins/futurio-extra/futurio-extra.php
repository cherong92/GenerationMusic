 ...
 * Return theme
 */
function futurio_extra_theme($atheme = '') {
	$theme = wp_get_theme();
...
if (wp_get_theme()->template == ('futurio') || wp_get_theme()->template == ('futurio-storefront')) {
} else {
	return;
}
...
function futurio_extra_scripts() {
    wp_enqueue_style('futurio-extra', plugin_dir_url(__FILE__) . 'assets/css/style.css', array(), FUTURIO_EXTRA_CURRENT_VERSION);
    wp_enqueue_script('futurio-extra-js', plugin_dir_url(__FILE__) . 'assets/js/futurio-extra.js', array('jquery'), FUTURIO_EXTRA_CURRENT_VERSION, true);
}
...
function futurio_extra_customize_enqueue() {
    wp_enqueue_style('futurio-extra-customizer', plugin_dir_url(__FILE__) . 'assets/css/admin/customizer.css', array(), FUTURIO_EXTRA_CURRENT_VERSION);
    wp_enqueue_style('font-awesome-css', get_template_directory_uri() . '/css/font-awesome.min.css', array(), '4.7.0');
}
...
function futurio_extra_action() {
	...
	remove_action('futurio_storefront_generate_footer_credits', 'futurio_storefront_generate_construct_footer');
	...
	add_action('futurio_storefront_generate_footer_credits', 'futurio_extra_generate_construct_footer');
    add_action('futurio_storefront_header_body', 'futurio_extra_preloader');
}
...
if (!class_exists('Kirki')) {
    include_once( plugin_dir_path(__FILE__) . 'inc/framework/kirki.php' );
}
...
function futurio_extra_activate() {
	...
	add_option( 'kirki_notices', array('discount_notice' => 1) );
}
